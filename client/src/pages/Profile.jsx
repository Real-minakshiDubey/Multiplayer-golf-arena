...
export default function Profile() {
  const { user } = useAuth();
  const [adminStats, setAdminStats] = useState({ challenges: 0 });

  useEffect(() => {
    if (user?.role === 'admin') {
      // Fetch admin specific stats if needed
      api.get('/challenges').then(res => {
        setAdminStats({ challenges: res.data.length });
      }).catch(() => {});
    }
  }, [user]);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12 bg-mesh min-h-screen">
      ...
      >
        {/* Background Glows */}
        <div className={`absolute top-0 right-0 w-64 h-64 ${isAdmin ? 'bg-accent-purple/10' : 'bg-golf-400/10'} blur-[100px] rounded-full -mr-32 -mt-32`} />
        <div className={`absolute bottom-0 left-0 w-64 h-64 ${isAdmin ? 'bg-accent-pink/10' : 'bg-accent-pink/10'} blur-[100px] rounded-full -ml-32 -mb-32`} />

        <div className="relative flex flex-col md:flex-row items-center gap-8 md:gap-12">
          {/* Avatar Area */}
          <div className="relative">
            <div className={`w-32 h-32 md:w-40 md:h-40 rounded-full bg-dark-950 border-4 ${isAdmin ? 'border-accent-purple/30' : 'border-white/5'} flex items-center justify-center text-7xl md:text-8xl shadow-2xl relative z-10`}>
              {user.avatar}
            </div>
            <motion.div 
              animate={{ rotate: 360 }}
              transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
              className={`absolute -inset-2 border-2 border-dashed ${isAdmin ? 'border-accent-purple/50' : 'border-golf-400/30'} rounded-full`}
            />
            <div className={`absolute -bottom-2 -right-2 ${isAdmin ? 'bg-accent-purple' : 'bg-golf-400'} text-white px-4 py-1.5 rounded-full text-[10px] font-black tracking-widest uppercase shadow-lg z-20`}>
              {isAdmin ? 'Architect' : 'Pro'}
            </div>
          </div>
          ...
          <div className="flex-1 text-center md:text-left">
            <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4 mb-4">
              ...
              <div className="flex items-center justify-center md:justify-start gap-2">
                <span className={`px-4 py-1.5 rounded-full bg-white/5 border border-white/10 text-[10px] font-black tracking-[0.2em] uppercase ${isAdmin ? 'text-accent-purple' : 'text-gray-400'}`}>
                  {isAdmin ? 'System Administrator' : `Rank #${user.elo >= 1500 ? 'Elite' : 'Challenger'}`}
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-6 text-sm text-gray-500 font-medium mb-8">
              ...
              <div className="flex items-center gap-2">
                <Shield className={`w-4 h-4 ${isAdmin ? 'text-accent-purple' : 'text-accent-purple'}`} />
                {isAdmin ? 'Root Access' : 'Verified Player'}
              </div>
              <div className="flex items-center gap-2">
            ...
            {/* Admin/Player Sub-header */}
            <div className="max-w-md mx-auto md:mx-0">
              {isAdmin ? (
                <div className="flex items-center gap-3 py-3 px-6 bg-accent-purple/10 rounded-2xl border border-accent-purple/20 w-fit">
        ...
        {isAdmin ? (
          <>
            ...
            <StatCard 
              ...
              label="System Node" 
              value="STABLE" 
              subValue="MULTI-TIER FALLBACK ENABLED"
              color="green-400"
              delay={0.3}
            />
            <StatCard 
