...
import ArenaSensei from '../components/ArenaSensei';
...
export default function ProblemDetail() {
  ...
  const [senseiAnalysis, setSenseiAnalysis] = useState(null);
  ...
  const handleConsultSensei = async () => {
    ...
    try {
      ...
      const res = await api.post('/ai/analyze', {
        ...
        userCode: code,
        language,
        error: firstFailed ? `Failed test case: input ${firstFailed.input}, expected ${firstFailed.expected}, but got ${firstFailed.actual}` : null
      });
      setSenseiAnalysis(res.data);
    } catch (err) {
      toast.error('The Sensei is meditating. Try again later.');
      setSenseiOpen(false);
    } finally {
              ...
              <div className="space-y-3">
                ...
                {testResults && (
                  <div className="glass rounded-sm p-4 space-y-2 border border-dark-600">
                    ...
                    {/* Consult Sensei Button */}
                    <button
                      onClick={handleConsultSensei}
                      className="w-full mt-2 py-3 bg-accent-purple/10 text-accent-purple rounded-sm font-black text-xs tracking-[0.2em] hover:bg-accent-purple/20 transition border border-accent-purple/20 flex items-center justify-center gap-2 group"
                    >
                      <Brain className="w-4 h-4 group-hover:animate-pulse" />
                      CONSULT ARENA SENSEI
                    </button>
                  </div>
                )}
              </div>
